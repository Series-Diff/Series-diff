...
'''
    ...
    def _execute_in_docker(
        ...
        try:
            ...
            docker_cmd = [
                ...
                "-v", f"{executor_path}:/executor.py:ro",  # Mount executor script
                self.EXECUTOR_IMAGE,
                "python", "/executor.py"
            ]
            ...
            return {"error": f"Execution timed out after {self.TIMEOUT_SECONDS} seconds"}
        except Exception as e:
    ...
    def _execute_restricted(
        ...
        try:
            ...
            if not isinstance(result, (int, float)):
                return {"error": f"Result must be a number, got {type(result).__name__}"}
            return {"result": float(result)}
        except Exception as e:
            return {"error": f"Calculation error: {str(e)}"}

    def _execute_basic_sandbox(
            self,
            code: str,
            series1: dict,
            series2: dict
        ...
        if "def calculate(" not in code:
            return {"error": "Plugin must define 'calculate(series1, series2)' function"}
        ...
        def safe_import(name, globals=None, locals=None, fromlist=(), level=0):
            ...
            raise ImportError(f"Import of '{name}' is not allowed. Only 'pandas' and 'numpy' are permitted.")
        ...
        namespace = {
            ...
            "__builtins__": {
                ...
                "tuple": tuple, "zip": zip, "map": map,
                "filter": filter, "enumerate": enumerate,
