...
describe('fetchAllRollingMeans', () => {
  ...
  it('handles malformed responses and errors gracefully', async () => {
    global.fetch = jest.fn()
      .mockResolvedValueOnce({ ok: true, json: () => Promise.resolve({ rolling_mean: null }) })  // Unexpected structure
      .mockResolvedValueOnce({ ok: true, json: () => Promise.resolve({ rolling_mean: { ts: 'not_number' } }) })  // No valid points
  ...
  it('skips non-array series data', async () => {
    global.fetch = jest.fn()
      .mockResolvedValueOnce({ ok: true, json: () => Promise.resolve({ rolling_mean: 'invalid' }) })  // Unexpected structure
      .mockResolvedValueOnce({ ok: true, json: () => Promise.resolve({ rolling_mean: 'invalid' }) })
      .mockResolvedValueOnce({ ok: true, json: () => Promise.resolve({ rolling_mean: 'invalid' }) });
  ...
  it('handles empty series map', async () => {
    global.fetch = jest.fn()
      .mockResolvedValueOnce({ ok: true, json: () => Promise.resolve({ rolling_mean: {} }) })  // Empty object
      .mockResolvedValueOnce({ ok: true, json: () => Promise.resolve({ rolling_mean: {} }) })
      .mockResolvedValueOnce({ ok: true, json: () => Promise.resolve({ rolling_mean: {} }) });
  ...
  it('handles non-object seriesMap', async () => {
    global.fetch = jest.fn().mockResolvedValue({ ok: true, json: () => Promise.resolve(null) } as Response);  // seriesMap will be {}
