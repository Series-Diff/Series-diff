...
describe('fetchAllStdDevs', () => {
  ...
  it('should return standard deviations correctly', async () => {
    global.fetch = jest.fn()
      .mockResolvedValueOnce({ ok: true, json: () => Promise.resolve({ standard_deviation: 45.3 }) })
      .mockResolvedValueOnce({ ok: true, json: () => Promise.resolve({ standard_deviation: 120.1 }) })
      .mockResolvedValueOnce({ ok: true, json: () => Promise.resolve({ standard_deviation: 118.7 }) });
