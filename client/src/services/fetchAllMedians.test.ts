...
describe('fetchAllMedians', () => {
  ...
  it('should fetch medians correctly for all files', async () => {
    global.fetch = jest.fn()
      .mockResolvedValueOnce({ ok: true, json: () => Promise.resolve({ median: 1013.25 }) })
      .mockResolvedValueOnce({ ok: true, json: () => Promise.resolve({ median: 1012.8 }) })
      .mockResolvedValueOnce({ ok: true, json: () => Promise.resolve({ median: 5.6 }) });
  ...
  it('should skip null/undefined medians and handle errors', async () => {
    global.fetch = jest.fn()
      .mockResolvedValueOnce({ ok: true, json: () => Promise.resolve({ median: null }) })
      .mockResolvedValueOnce({ ok: false, text: () => Promise.resolve('404') })
