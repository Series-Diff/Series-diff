...
describe('fetchAllVariances', () => {
  ...
  it('fetches variances successfully', async () => {
    global.fetch = jest.fn()
      .mockResolvedValueOnce({ ok: true, json: () => Promise.resolve({ variance: 4.2 }) })
      .mockResolvedValueOnce({ ok: true, json: () => Promise.resolve({ variance: 5.1 }) })
      .mockResolvedValueOnce({ ok: true, json: () => Promise.resolve({ variance: 12.8 }) });
