...
describe('fetchAllDifferences', () => {
  ...
  it('returns correctly structured differences', async () => {
    ...
    global.fetch = jest.fn().mockResolvedValue({ ok: true, json: () => Promise.resolve(mockDiff) });
  ...
  it('handles non-ok responses', async () => {
    ...
    const result = await fetchAllDifferences(input, 5);
    expect(result.temp).toEqual({});
    expect(console.error).toHaveBeenCalledWith(expect.stringContaining('Failed to fetch difference'), 'Server error');
  });
  ...
  it('handles tolerance null', async () => {
    global.fetch = jest.fn().mockResolvedValue({ ok: true, json: () => Promise.resolve({ difference: {} }) });
