...
describe('fetchAllAutoCorrelations', () => {
  ...
  it('calculates autocorrelations properly', async () => {
    global.fetch = jest.fn()
      .mockResolvedValueOnce({ ok: true, json: () => Promise.resolve({ autocorrelation: 0.87 }) })
      .mockResolvedValueOnce({ ok: true, json: () => Promise.resolve({ autocorrelation: 0.92 }) })
      .mockResolvedValueOnce({ ok: true, json: () => Promise.resolve({ autocorrelation: 0.75 }) });
  ...
  it('handles null values and API failures', async () => {
    global.fetch = jest.fn()
      .mockResolvedValueOnce({ ok: true, json: () => Promise.resolve({ autocorrelation: null }) })
