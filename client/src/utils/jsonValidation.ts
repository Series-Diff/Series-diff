...
// We previously relied on Ajv for schema validation. To avoid runtime issues when Ajv
// fails to initialize in some environments, we now perform lightweight manual
// structure checks instead.
...
export function validateTimeSeriesJSON(data: unknown): ValidationResult {
  ...
  // Step 2: Basic structure validation (must be a non-empty array)
