...
export const DataImportPopup = forwardRef<DataImportPopupHandle, Props>(
  ({ show, files, onHide, onComplete }, ref) => {
    ...
    const [appliedPivotColumns, setAppliedPivotColumns] = useState<Record<string, string>>({}); // Store which column was used for pivot per file
    ...
    useEffect(() => {
      if (columnOptions.length > 0) {
        ...
        // Prioritize "value" patterns for Value to Aggregate
        const probableValue = columnOptions.find(c => 
          /value|amount|metric|measurement|val|num/i.test(c)
        ) || columnOptions.find(c => c.toLowerCase().includes('value')) || columnOptions[columnOptions.length - 1];
        
        // Prioritize "data_type" patterns for Pivot Column
        const probableCategory = columnOptions.find(c => 
          /data_?type|datatype|category|type|group|class/i.test(c)
        ) || columnOptions.find(c => c !== probableDate && c !== probableValue) || columnOptions[1] || columnOptions[0];
    ...
    const handleApplyPivot = async () => {
      ...
      try {
        ...
        if (flattenedData.length > 0) {
          ...
          // Save the column that was used for pivoting before resetting
          setAppliedPivotColumns(prev => ({ ...prev, [fileId]: pivotColumn }));
      ...
      Object.entries(fileConfigs).forEach(([fileKey, config]) => {
        ...
        config.rawData.forEach(row => {
          ...
          groups.forEach(group => {
            ...
            // Skip only if value is actually undefined or null, allow 0 and other falsy numbers
            if (value === undefined || value === null || value === '') return;
            
            // Accept any value that can be parsed as a number
            const numericValue = parseFloat(value);
            if (!isNaN(numericValue)) {
              result[isoDateString][group.name][fileKey] = numericValue;
            }
          });
        });
      });
    ...
    // --- HELPER: Check if column contains only null/undefined/empty values ---
    const isColumnEmpty = (fileKey: string, columnName: string): boolean => {
      const config = fileConfigs[fileKey];
      if (!config?.rawData || config.rawData.length === 0) return true;

      // Check all rows to see if column has any non-null values
      return config.rawData.every(row => {
      ...
      <Modal show={show} onHide={onHide} backdrop="static" keyboard={false} size="xl" centered scrollable>
        ...
        <Modal.Body ref={modalBodyRef} className="d-flex flex-column gap-3">
          {currentStep === 'file-preview' ? (
            <>
              <Form.Group>
                <Form.Label className="fw-bold d-flex align-items-center gap-2 mb-0">
                  File:
                  {currentFile ? (
                    editingFileName ? (
                      <>
                      ...
                      onChange={(e) => {
                        ...
                        if (!e.target.checked) {
                          if (originalData[fileId] && originalData[fileId].length > 0) {
                            ...
                            // Clear pivot selections to allow auto-detect to work
                            setPivotIndex('');
                            setPivotColumn('');
                            setPivotValue('');
                ...
                <div className="d-flex flex-column gap-3">
                  ...
                  {isPivotMode[getFileId(currentFile)] && currentConfig.rawData.length > 0 && (
                    <p className="text-muted small m-0">
                      {pivotApplied[getFileId(currentFile)] && appliedPivotColumns[getFileId(currentFile)]
                        ? <>* New columns were generated from <strong>{appliedPivotColumns[getFileId(currentFile)]}</strong>.</>
                        : pivotColumn
                        ? <>* New columns will be generated from <strong>{pivotColumn}</strong> when you apply pivot.</>
                        : <>* Select columns above and click Apply to pivot data.</>
                      }
                    </p>
                  )}
                </div>
                ...
                <>
                  ...
                  {groups.map((group) => {
                    ...
                    return (
                      <div
                        key={group.id}
                        className={`p-3 border rounded d-flex flex-column gap-2 ${group.id === 'date' ? 'bg-info bg-opacity-10 border-info border-opacity-50' : 'bg-white'}`}
                      >
                        ...
                        {Object.keys(fileConfigs).map((fileKey) => {
                          ...
                          // Filter out columns that are used or empty
                          const availableColumns = fileColumns.filter(col => {
                            const isCurrentSelection = col === currentSelection;
                            const isUsed = usedColumns.includes(col);
                            const isEmpty = isColumnEmpty(fileKey, col);
                            // Allow any non-empty, non-used column (including sparse data)
                            return isCurrentSelection || (!isUsed && !isEmpty);
                          });
                          const errorKey = `${group.id}-${fileKey}`;
                          const hasError = !!fieldErrors[errorKey];

                          return (
                            <Form.Group key={`${group.id}-${fileKey}`}>
                              <div className="d-flex align-items-center gap-3">
                                ...
                                <div className="flex-grow-1">
                                  <Form.Select
                                    ...
                                    size="sm"
                                  >
                                    {group.id !== 'date' && <option value="none">None</option>}
